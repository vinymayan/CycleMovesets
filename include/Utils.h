...
namespace GlobalControl {
    ...
    const std::pair<RE::INPUT_DEVICE, SkyPromptAPI::ButtonID> key_U = {RE::INPUT_DEVICE::kKeyboard, 21};
    ...
    const std::pair<RE::INPUT_DEVICE, SkyPromptAPI::ButtonID> key_O = {
        RE::INPUT_DEVICE::kKeyboard, 24};  // Assumi que "diminuir com U" foi um erro de digitação e usei a tecla O.

    // Criamos os prompts. Eles não precisam de texto, pois são apenas listeners.
    // ActionID 0: Incrementar
    const SkyPromptAPI::Prompt prompt_Increment("", 0, 0, SkyPromptAPI::PromptType::kSinglePress, 0,
                                                std::span(&key_U, 1));
    // ActionID 1: Resetar
    const SkyPromptAPI::Prompt prompt_Reset("", 0, 1, SkyPromptAPI::PromptType::kSinglePress, 0, std::span(&key_I, 1));
    // ActionID 2: Decrementar
    const SkyPromptAPI::Prompt prompt_Decrement("", 0, 2, SkyPromptAPI::PromptType::kSinglePress, 0,
                                                std::span(&key_O, 1));
